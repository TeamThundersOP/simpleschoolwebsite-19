  ...
  return (
    <section className={`${TOKENS.SECTION_PADDING} ${className}`}>
      <div className={TOKENS.CONTAINER_CLASS}>
        <div
          className={`flex items-center gap-8 lg:gap-20 w-full max-lg:flex-col max-lg:gap-8 ${align === "right" ? "lg:flex-row-reverse" : ""}`}
        >
          <motion.div
          ...
          >
            <div className="flex flex-col items-start gap-4 lg:gap-6 w-full">
              <h2 className={`w-full ${TOKENS.H2_CLASS} ${textColor}`}>{title}</h2>
              <div className={`w-full ${TOKENS.BODY_CLASS} ${textColor}`}>{body}</div>
            </div>
            {ctaLabel && ctaHref && (
              <a
                href={ctaHref}
              ...
              >
                <span className="text-white font-medium">{ctaLabel}</span>
              </a>
            )}
          </motion.div>
          <motion.div
