...
const Footer: React.FC = () => {
    ...
    if (link.href.startsWith('#')) {
      return (
        <a
          key={link.href}
          href={link.href}
          className="text-white/80 hover:text-[#b2e61b] transition-colors duration-300 text-sm font-medium"
        >
          {link.name}
        </a>
      );
  ...
  return (
    <footer className="bg-[#021223] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        {/* Main Footer Content */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12">
          {/* Company Info - Takes full width on mobile, half on tablet, 4 columns on desktop */}
          <div className="lg:col-span-4 space-y-6">
            <Link to="/">
              <img 
                src="/lovable-uploads/af002aad-4a7-49c6-bd8f-d66970d17721.png" 
          ...
          {/* Navigation Links - Grid layout for better organization */}
          <nav className="lg:col-span-5" aria-label="Footer Navigation">
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-3 gap-6 lg:gap-8">
              {/* Products */}
        ...
        <div className="mt-12 pt-8 border-t border-white/10">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-wrap justify-center sm:justify-end gap-6 text-xs sm:text-sm">
              <Link 
                to="/privacy" 
                className="text-white/60 hover:text-[#b2e61b] transition-colors duration-300"
              >
                Privacy Policy
              </Link>
              <Link 
