export const TOKENS = {
  ...
  H1_CLASS:
    "text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-[52px] font-medium leading-tight tracking-tight",
  ...
  BTN_PRIMARY_CLASS:
    "inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 rounded-md font-medium transition-colors",
  BTN_SECONDARY_CLASS:
    "inline-flex items-center justify-center gap-2 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 rounded-md font-medium transition-colors",

  // Custom button from Products page
  BTN_CUSTOM_PRIMARY:
    "flex justify-center items-center gap-2 border cursor-pointer bg-[#0C0504] text-white px-4 py-2.5 rounded-full border-solid border-[#0C0504] transition-all duration-300 hover:bg-blue-600 hover:border-blue-600 hover:shadow-lg hover:scale-105",
  ...
  CARD_PRODUCT:
    "flex flex-col items-start gap-6 lg:gap-8 flex-1 group transition-all duration-300 hover:-translate-y-2 hover:shadow-xl rounded-2xl hover:bg-gray-50/50 p-4 -m-4",
};
